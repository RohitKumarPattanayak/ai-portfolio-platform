          ...
          return (
            <div key={idx} className="relative pl-8 md:pl-10 group">
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-4">
                <div>
                  <h3 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{role}</h3>
                  <h4 className="text-lg text-emerald-600/90 dark:text-emerald-500/80 font-medium tracking-wide mt-1 transition-colors">{company}</h4>
                </div>
                <span className="inline-flex items-center text-sm px-3.5 py-1.5 bg-gray-100/80 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-700/50 rounded-full text-gray-600 dark:text-gray-400 shrink-0 shadow-sm backdrop-blur-sm whitespace-nowrap transition-colors">
                  {start_date} - {end_date}
                </span>
              </div>
              <p className="text-gray-600 dark:text-gray-400/90 leading-relaxed text-base whitespace-pre-wrap font-light transition-colors">
                {description}
              </p>
            </div>
          );
