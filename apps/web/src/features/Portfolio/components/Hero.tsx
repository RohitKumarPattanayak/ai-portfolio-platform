...
const Hero = memo(({ data }: HeroProps) => {
  ...
  return (
    <section className="relative flex flex-col-reverse md:flex-row items-center md:items-start lg:items-center gap-10 md:gap-12 py-16 md:py-24 px-6 max-w-5xl mx-auto w-full border-b border-gray-200 dark:border-gray-800/60 transition-colors duration-300">
      <div className="flex-1 space-y-5 flex flex-col items-center md:items-start text-center md:text-left">
        ...
        {intro && (
            <p className="text-base md:text-lg lg:text-xl text-gray-600 dark:text-gray-400 leading-relaxed max-w-2xl font-light transition-colors duration-300">
              {intro}
            </p>
        )}
        <div className="flex flex-wrap justify-center md:justify-start gap-4 text-gray-600 dark:text-gray-400 mt-2">
          {exp && (
              <span className="px-4 py-1.5 bg-white dark:bg-[#0F172A] rounded-full border border-gray-200 dark:border-gray-700/50 text-xs md:text-sm font-medium shadow-sm transition-colors duration-300">
                  {exp} Experience
              </span>
          )}
        </div>
        <div className="flex items-center justify-center md:justify-start gap-4 md:gap-5 pt-4 md:pt-6">
          {personalInfo.email && (
      ...
      {pic && (
        <div className="w-36 h-36 md:w-60 md:h-60 lg:w-72 lg:h-72 relative shrink-0 group mx-auto md:mx-0">
          <div className="absolute inset-0 bg-gradient-to-tr from-blue-400 to-emerald-400 dark:from-blue-500 dark:to-emerald-500 rounded-full blur-[20px] md:blur-2xl opacity-40 dark:opacity-20 group-hover:opacity-60 dark:group-hover:opacity-40 transition-opacity duration-700 animate-pulse"></div>
          <img 
            src={pic} 
